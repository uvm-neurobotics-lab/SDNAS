...
def main(xargs):
  ...
  if xargs.model_config is None and not args.constrain:
    model_config = dict2config({'name': 'ProxylessNAS', 'C': xargs.channel, 'N': xargs.num_cells,
  ...
  logger.log('ProxylessNAS : run {:} epochs, cost {:.1f} s, last-geno is {:}.'.format(total_epoch, search_time.sum, genotypes[total_epoch-1]))
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser("ProxylessNAS")
