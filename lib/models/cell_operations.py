...
def conv_cost(C_in, C_out, kernel_size, im_size):
  FLOP = im_size**2 * kernel_size**2 * C_in * C_out
  MAC = im_size**2 * (C_in + C_out)
  MAC += (kernel_size**2 * C_in * C_out)
  return (FLOP + MAC)

def pool_cost(C_in, C_out, kernel_size, im_size):
  FLOP = im_size**2 * kernel_size**2 * C_in * C_out
  MAC = im_size**2 * (C_in + C_out)
  return (FLOP + MAC)
...
COSTS = {
  'none'         : lambda C_in, C_out, size: 0,
  'avg_pool_3x3' : lambda C_in, C_out, size: pool_cost(C_in, C_out, 3, size),
  'max_pool_3x3' : lambda C_in, C_out, size: pool_cost(C_in, C_out, 3, size),
  'nor_conv_7x7' : lambda C_in, C_out, size: conv_cost(C_in, C_out, 7, size),
  'nor_conv_3x3' : lambda C_in, C_out, size: conv_cost(C_in, C_out, 3, size),
  'nor_conv_1x1' : lambda C_in, C_out, size: conv_cost(C_in, C_out, 1, size),
  'dua_sepc_3x3' : lambda C_in, C_out, size: (conv_cost(C_in, C_in, 3, size) + conv_cost(C_in, C_out, 1, size))*2,
  'dua_sepc_5x5' : lambda C_in, C_out, size: (conv_cost(C_in, C_in, 5, size) + conv_cost(C_in, C_out, 1, size))*2,
  'dil_sepc_3x3' : lambda C_in, C_out, size: (conv_cost(C_in, C_in, 3, size) + conv_cost(C_in, C_out, 1, size)),
  'dil_sepc_5x5' : lambda C_in, C_out, size: (conv_cost(C_in, C_in, 5, size) + conv_cost(C_in, C_out, 1, size)),
  'skip_connect' : lambda C_in, C_out, size: (C_in + C_out)*size**2,
}
